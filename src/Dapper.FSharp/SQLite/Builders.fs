[<AutoOpen>]
module Dapper.FSharp.SQLite.Builders
...
type TableMapping = { Name: string  }

/// Fully qualifies a column with: {table}.{column}
let private fullyQualifyColumn (tables: Map<FQName, TableMapping>) (property: Reflection.MemberInfo) =
    ...
    sprintf "%s.%s" tbl.Name property.Name
...
type SelectExpressionBuilder<'T>() =
    ...
    member this.For (state: QuerySource<'T>, f: 'T -> QuerySource<'T>) =
        ...
        QuerySource<'T, SelectQuery>({ query with Table = tbl.Name }, state.TableMappings)
    ...
    member this.Skip (state:QuerySource<'T>, skip, take) = 
        let query = state |> getQueryOrDefault
        QuerySource<'T, SelectQuery>({ query with Pagination = { query.Pagination with Skip = skip; Take = take } }, state.TableMappings)
    
    /// Sets the TAKE value for query
    [<CustomOperation("take", MaintainsVariableSpace = true)>]
    member this.Take (state:QuerySource<'T>, skip, take) = 
        let query = state |> getQueryOrDefault
        QuerySource<'T, SelectQuery>({ query with Pagination = { query.Pagination with Skip = skip; Take = take } }, state.TableMappings)
...
type DeleteExpressionBuilder<'T>() =
    ...
    member this.For (state: QuerySource<'T>, f: 'T -> QuerySource<'T>) =
        let tbl = state.GetOuterTableMapping()
        let query = state |> getQueryOrDefault
        QuerySource<'T, DeleteQuery>({ query with Table = tbl.Name }, state.TableMappings)
...
type InsertExpressionBuilder<'T>() =
    ...
    member this.For (state: QuerySource<'T>, f: 'T -> QuerySource<'T>) =
        let tbl = state.GetOuterTableMapping()
        let query = state |> getQueryOrDefault
        QuerySource<'T, InsertQuery<'T>>({ query with Table = tbl.Name }, state.TableMappings)
...
type UpdateExpressionBuilder<'T>() =
    ...
    member this.For (state: QuerySource<'T>, f: 'T -> QuerySource<'T>) =
        let tbl = state.GetOuterTableMapping()
        let query = state |> getQueryOrDefault
        QuerySource<'T, UpdateQuery<'T>>({ query with Table = tbl.Name }, state.TableMappings)
